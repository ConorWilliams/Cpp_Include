...
class ByteOfBits {
   ...
   public:
    ...
    // function to lookup how many bits are set to one in byte in the lookup
    // table
...
class DenseBitsH {
   public:
    static const uint32_t length = (size >> 3) + ((size & MASK) != 0);
    ...
    // functions to set val in bit^th position to 1
    inline void high(uint32_t bit) {
        bit %= size;
        set[bit >> 3].high(bit & MASK);
    }

    // functions to set val in bit^th position to 0
    inline void low(uint32_t bit) {
        bit %= size;
        set[bit >> 3].low(bit & MASK);
    }

    // functions to swap val in bit^th position
    inline void flip(uint32_t bit) {
        bit %= size;
    ...
    void print(void) {
        cout << PRINTER[set[0].test(0)];
        for (uint32_t i = 1; i < size; ++i) {
            cout << PRINTER[2] << PRINTER[set[i >> 3].test(i & MASK)];
        }
        cout << "\n";
    }
